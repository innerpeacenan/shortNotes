...
class ActiveRecord
{
    ...
    {
        if (null === ($masterDb = Container::getSingleton('master'))) {
            $pool = Config::get('main/db.master');
            shuffle($pool);
            $config = reset($pool);
            $config['params'][] = config::get('main/db.shareParam');
            $masterDb = Container::get($config);
            Container::setSingletons('master', $masterDb);
        }
        return $masterDb;
    }
    ...
    {
        if (null === ($slaveDb = Container::getSingleton('slave'))) {
            $pool = Config::get('main/db.slave');
            shuffle($pool);
            $config = reset($pool);
            $config['params'][] = config::get('main/db.shareParam');
            $slaveDb = Container::get($config);
            Container::setSingletons('slave', $slaveDb);
        }
        return $slaveDb;
    }
    ...
    public function setAttributes(array $columns)
    {
        foreach ($columns as $name => $value) {
            // 如主键已近存在，或者字段不在表结构是,则直接过滤(与表结构比,而不是与旧有值比较)
            if (in_array($name, $this->_primaryKey) and (!array_key_exists($name, $this->_columns))) {
                continue;
            }
            $this->$name = $value;
        }
    ...
    {
        /** @var static $ar */
        $ar = Container::get(['class' => get_called_class()]);
